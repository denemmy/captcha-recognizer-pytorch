    ...
    def set_states(self, states):
        for k, v in states.items():
            if k == 'metrics':
                ...
                for metric_name_new, values_new in v.items():
                    if metric_name_new in self.metrics:
                        values_prev = self.metrics[metric_name_new]
                        values_new['console_event']['period'] = values_prev['console_event']['period']
                        values_new['tb_event']['period'] = values_prev['tb_event']['period']
                    self.metrics[metric_name_new] = values_new
    ...
    def _check_metrics(self):
        ...
        for metric_name, metric in self.metrics.items():
            ...
            if self.tb_writer is not None and self._check_event(metric['tb_event']):
                ...
                self.tb_writer.add_scalar(metric['tb_name'], value, global_step=global_step)
